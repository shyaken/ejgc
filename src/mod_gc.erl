...
-define(PROCNAME, ejabberd_mod_gc).
...
shutdown_rooms(Host) ->
    MyHost = gen_mod:get_module_opt_host(Host, mod_gc,
					 <<"groupchat.@HOST@">>),
...
%% C) mod_gc:stop was called, and each room is being terminated
%%    In this case, the mod_gc process died before the room processes
...
%% Else use the passed options as defined in mod_gc_room.
...
start_supervisor(Host) ->
    Proc = gen_mod:get_module_proc(Host,
				   ejabberd_mod_gc_sup),
    ChildSpec = {Proc,
		 {ejabberd_tmp_sup, start_link, [Proc, mod_gc_room]},
			...
			{<<"presence">>, <<"">>} ->
				...
				true ->
				    {ok, Pid} = start_new_room(
						  ...
						  Nick, DefRoomOpts),
				    register_room(Host, Room, Pid),
				    mod_gc_room:route(Pid, From, Nick, Packet),
				    ok;
				false ->
				    Lang = xml:get_attr_s(<<"xml:lang">>, Attrs),
...
start_new_room(Host, ServerHost, Access, Room,
    ...
    case restore_room(ServerHost, Host, Room) of
	    ...
	    mod_gc_room:start(Host, ServerHost, Access,
			       Room, HistorySize,
			       RoomShaper, From,
			       Nick, DefRoomOpts);
        Opts ->
...
iq_get_vcard(Lang) ->
    [#xmlel{name = <<"FN">>, attrs = [],
	    children = [{xmlcdata, <<"ejabberd/mod_gc">>}]},
