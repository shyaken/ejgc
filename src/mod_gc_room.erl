...
-else.
-define(SUPERVISOR_START, 
	Supervisor = gen_mod:get_module_proc(ServerHost, ejabberd_mod_gc_sup),
...
init([Host, ServerHost, Access, Room, HistorySize, RoomShaper, Creator, _Nick, DefRoomOpts]) ->
    ...
    if (State1#state.config)#config.persistent ->
	   mod_gc:store_room(State1#state.server_host,
			      State1#state.host,
			      State1#state.room,
			      make_opts(State1));
       true -> ok
	  ...
	  case xml:get_attr_s(<<"type">>, Attrs) of
		...
		MinMessageInterval =
		    trunc(gen_mod:get_module_opt(StateData#state.server_host,
						 mod_gc, min_message_interval, fun(MMI) when is_number(MMI) -> MMI end, 0)
                          * 1000000),
				   ...
				   case get_affiliation(IJID, StateData) of
					 ...
					 case
					   (NSD#state.config)#config.persistent
					     of
					   true ->
					       mod_gc:store_room(NSD#state.server_host,
								  NSD#state.host,
								  NSD#state.room,
								  make_opts(NSD));
					   _ -> ok
...
terminate(Reason, _StateName, StateData) ->
    ...
    mod_gc:room_destroyed(StateData#state.host, StateData#state.room, self(),
			 ...
			 true ->
			     case is_nick_change(From, Nick, StateData) of
				   ...
				   case {nick_collision(From, Nick, StateData),
					 mod_gc:can_use_nick(StateData#state.server_host,
...
get_max_users_admin_threshold(StateData) ->
    gen_mod:get_module_opt(StateData#state.server_host,
			   mod_gc, max_users_admin_threshold,
                           fun(I) when is_integer(I), I>0 -> I end,
                           5).
	...
	of
	  ...
	  MessageShaper =
	      shaper:new(gen_mod:get_module_opt(StateData#state.server_host,
						mod_gc, user_message_shaper,
                                                fun(A) when is_atom(A) -> A end,
						none)),
	  PresenceShaper =
	      shaper:new(gen_mod:get_module_opt(StateData#state.server_host,
						mod_gc, user_presence_shaper,
                                                fun(A) when is_atom(A) -> A end,
						none)),
...
is_allowed_log_change(XEl, StateData, From) ->
	...
	of
      ...
      true ->
	  allow ==
	    mod_gc_log:check_access_log(StateData#state.server_host,
					 From)
    end.
...
change_config(Config, StateData) ->
	...
	of
      ...
      {true, false} ->
	  mod_gc:forget_room(NSD#state.server_host,
			      NSD#state.host, NSD#state.room);
      {false, false} -> ok
    end,
...
destroy_room(DEl, StateData) ->
    ...
    case (StateData#state.config)#config.persistent of
      true ->
	  mod_gc:forget_room(StateData#state.server_host,
...
add_to_log(Type, Data, StateData)
    ...
    mod_gc_log:add_to_log(StateData#state.server_host,
